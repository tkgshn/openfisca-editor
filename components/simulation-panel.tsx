...
export function SimulationPanel({ institution }: SimulationPanelProps) {
  ...
  const isChildcareGrant = useMemo(() => {
    return institution.name === "子育てめっちゃ頑張ってください助成金"
  }, [institution.name])
  ...
  const runSimulation = async (params: Record<string, number> = paramValues) => {
    setIsRunning(true)
    try {
      // 子育てめっちゃ頑張ってください助成金のシミュレーションを実行
      if (isChildcareGrant) {
  ...
  // 子育てめっちゃ頑張ってください助成金のシミュレーションデータを生成
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-muted/50 p-4 rounded-md">
                    <h4 className="text-sm font-medium mb-2">対象世帯数</h4>
                    <div className="flex items-end gap-2">
                      <span className="text-2xl font-bold">{simulationData.stats.eligibleCount}</span>
                  ...
                  <div className="bg-muted/50 p-4 rounded-md">
                    <h4 className="text-sm font-medium mb-2">総給付額</h4>
                    <div className="text-2xl font-bold">{formatCurrency(simulationData.stats.totalSupport)}</div>
                  </div>

                  <div className="bg-muted/50 p-4 rounded-md">
                    <h4 className="text-sm font-medium mb-2">平均給付額（対象世帯）</h4>
                ...
                <div className="border rounded-md overflow-hidden bg-white">
                  {typeof window !== "undefined" && (
                    <Plot
                      ...
                      layout={simulationData.plotData.layout}
