...
export function InstitutionDetails({ institution, onUpdate, onDelete }: InstitutionDetailsProps) {
  ...
  const testRunnerRef = useRef<HTMLButtonElement>(null)
    ...
    const runTests = async () => {
      try {
        ...
        if (!institutionTestResult) {
          const results = await runTestForInstitution(institution).catch((error) => {
  ...
  }, [institution, institutionTestResult, runTestForInstitution, onUpdate])
...
`
  ...
  return (
    <div className="space-y-4 pt-24">
      ...
      {/* Test Runner Panel */}
      {/* <TestRunnerPanel institution={institution} onTestComplete={setCurrentTestResults} /> */}
      ...
      {/* Hidden TestRunnerPanel */}
      {/* <div className="hidden">
        <Button ref={testRunnerRef} id="hidden-test-runner-btn" className="hidden">
          Run
        </Button>
      </div> */}
    </div>
