...
async function initGitOnServer() {
    ...
    try {
        // サーバーサイドでのみ必要なモジュールを動的にインポート
        // Note: dynamic import() is used to ensure these modules are not bundled for client-side
        const { simpleGit } = await import(/* webpackIgnore: true */ 'simple-git');
...
// サーバーサイドの場合のみ初期化を実行（クライアントサイドでは実行しない）
if (isServer) {
    // 非同期で初期化を行う（実行時にエラーが発生しても致命的にならないようにする）
    setTimeout(() => {
        initGitOnServer().catch(error =>
            console.error('Git初期化中にエラーが発生しました:', error)
        );
    }, 0);
}
